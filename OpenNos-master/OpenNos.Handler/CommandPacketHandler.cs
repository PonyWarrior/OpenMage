...
namespace OpenNos.Handler
{
    public class CommandPacketHandler : IPacketHandler
    {
        ...
        public void CreateShell(CreateShellPacket createShellPacket)
        {
            if (createShellPacket != null)
            {
                short vnum = 0;
                switch (createShellPacket.Type)
                {
                    default:
                        Session.SendPacket(Session.Character.GenerateSay("Invalid Type", 10));
                        break;
                    case 1:
                        // Half
                        if (createShellPacket.SubType == 1) { vnum = 598; }
                        else if (createShellPacket.SubType == 2) { vnum = 664; }
                        else { Session.SendPacket(Session.Character.GenerateSay("Invalid SubType", 10)); return; }
                        break;
                    case 2:
                        // Full
                        if (createShellPacket.SubType == 1) { vnum = 567; }
                        else if (createShellPacket.SubType == 2) { vnum = 579; }
                        else { Session.SendPacket(Session.Character.GenerateSay("Invalid SubType", 10)); return; }
                        break;
                    case 3:
                        // Special
                        if (createShellPacket.SubType == 1) { vnum = 570; }
                        else if (createShellPacket.SubType == 2) { vnum = 582; }
                        else { Session.SendPacket(Session.Character.GenerateSay("Invalid SubType", 10)); return; }
                        break;
                    case 4:
                        // PvP
                        if (createShellPacket.SubType == 1) { vnum = 573; }
                        else if (createShellPacket.SubType == 2) { vnum = 585; }
                        else { Session.SendPacket(Session.Character.GenerateSay("Invalid SubType", 10)); return; }
                        break;
                    case 5:
                        // Perfect
                        if (createShellPacket.SubType == 1) { vnum = 576; }
                        else if (createShellPacket.SubType == 2) { vnum = 588; }
                        else { Session.SendPacket(Session.Character.GenerateSay("Invalid SubType", 10)); return; }
                        break;
                }
                if (vnum != 0)
                {
                    sbyte rarity = Convert.ToSByte(createShellPacket.Rarity);
                    ItemInstance inv = Session.Character.Inventory.AddNewToInventory(vnum, 1, InventoryType.Equipment, Rare: rarity, Upgrade: 0, Design: 0).FirstOrDefault();
                    if (inv != null)
                    {
                        WearableInstance shell = Session.Character.Inventory.LoadBySlotAndType<WearableInstance>(inv.Slot, inv.Type);
                        shell.Upgrade = createShellPacket.Level;
                    }
                    else
                    {
                        Session.SendPacket(Session.Character.GenerateSay("No space in inventory", 0));
                    }
                }
            }
            else
            {
                Session.SendPacket(Session.Character.GenerateSay(CreateShellPacket.ReturnHelp(), 10));
            }
        }
